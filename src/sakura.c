...
sakura_add_tab()
{
	...
	term->child_exited = g_signal_connect(G_OBJECT(term->vte), "child-exited", G_CALLBACK(sakura_child_exited), NULL);
...
sakura_del_tab(gint page)
{
	...
	g_signal_handler_disconnect(G_OBJECT(term->vte), term->child_exited);
